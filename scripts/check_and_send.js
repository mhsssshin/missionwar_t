...
async function sendTeamMissionWebhook(mission, teamKey) {
  ...
  const myTeamRealName = teamKey === 'team1' ? '한잔하시조' : '두고보시조';
  const oppositeTeamRealName = teamKey === 'team1' ? '두고보시조' : '한잔하시조';
  ...
  const payload = {
    ...
    content: `@everyone 🚨 **[기습 팀 미션 발동]** 🚨`,
    embeds: [{
      title: `🔥 [기습 팀 미션 발동] 세션 #${mission.sessionIndex}`,
      description: [
        `**${myTeamRealName} 시크릿 커맨더**로부터 기습 미션이 투척되었습니다!`,
        ``,
      ...
      footer: {
        text: `워크숍 미션전쟁 • ${myTeamRealName} (${teamName})`,
...
async function sendIndividualMissionWebhook(indMission, teamKey) {
  ...
  const payload = {
    ...
    embeds: [{
      title: `⚡ [개인 기습 지령 발동] (${indMission.participant} 대원)`,
      description: [
        `**${indMission.participant}** 요원에게 극비 개인 지령이 배정되었습니다!`,
        ``,
...
async function run() {
  ...
  // ─── 0. Check if a new deployment notice needs to be sent to Discord ───
  if (schedule.deployNoticePending) {
    const activeTeam = schedule.lastDeployedTeam || 'team1';
    const attackerTeam = activeTeam === 'team1' ? '두고보시조' : '한잔하시조';
    const targetTeam = activeTeam === 'team1' ? '한잔하시조' : '두고보시조';
    const deployNoticePayload = {
      username: `[작전 통제본부] ${attackerTeam}`,
      avatar_url: activeTeam === 'team1'
        ? "https://cdn-icons-png.flaticon.com/512/4436/4436481.png"
        : "https://cdn-icons-png.flaticon.com/512/4436/4436452.png",
      content: `@everyone 🚨 **[기습 미션 전쟁 선포]** 🚨`,
      embeds: [{
        title: `⚔️ [극비 작전 발령] ${attackerTeam} ➔ ${targetTeam} ⚔️`,
        description: [
          `🔥 **${attackerTeam}**가 **${targetTeam}**에게 족쇄가 될 극비 긴급 미션을 전격 부여했습니다!`,
          ``,
          `⚠️ **[시크릿 커맨더 긴급 하사 지령]**`,
          `언제, 어느 순간에 당신의 턱밑으로 기습 지령 카드가 투척될지 아무도 모릅니다.`,
          `발동 시각이 도래하는 즉시 모든 대원은 사력을 다해 최우선으로 지령을 완수해야 합니다!`,
          ``,
          `☠️ **지령 수행 거부 및 실패는 절대 용납되지 않으며, 참혹한 벌칙이 뒤따릅니다.**`,
          `단 한 명의 낙오자도 없이, 타깃이 된 미션을 반드시 수행하십시오! 💥`
        ].join('\n'),
        color: activeTeam === 'team1' ? 16722271 : 65535,
        footer: {
          text: `Secret Mission War • 극비 작전 통제 시스템`,
          icon_url: "https://cdn-icons-png.flaticon.com/512/3176/3176371.png"
        },
        timestamp: new Date().toISOString()
      }]
    ...
    console.log(`📢 Sending deploy notice webhook for ${attackerTeam} -> ${targetTeam}...`);
    const noticeSuccess = await postToDiscord(deployNoticePayload, `Deploy Notice: ${attackerTeam} -> ${targetTeam}`);
    if (noticeSuccess) {
      schedule.deployNoticePending = false;
